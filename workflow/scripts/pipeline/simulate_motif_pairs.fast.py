...
parser.add_argument('--blacklist', type=str, required=False, default=None)
...
# process WITHOUT the blacklist regions
...
# process WITH the blacklist regions
else:
...
for anchor_name1, anchor_name2 in filter_df[['anchor_name1', 'anchor_name2']].values:
    ...
    sim_anchors1 = np.random.choice(anchor_shuffle_dict[anchor_name1], size=sims, replace=True) 
    sim_anchors2 = np.random.choice(anchor_shuffle_dict[anchor_name2], size=sims, replace=True) 
